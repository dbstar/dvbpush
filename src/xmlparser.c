...
static int brand_xml_sqlite_insert(DBSTAR_BRAND_S *p_brand)
{
	...
	mid_push_regist(p_brand->pathid, total_xmlpath, (p_brand->totalsize));
...
static int allpid_sqlite_insert(int pid)
{
	...
	if(ret_sqlexec<=0){
		...
		int fid = alloc_filter((unsigned short)pid);
...
static int brand_unregist_sqlite_callback(char **result, int row, int column, void *receiver)
{
	...
	for(i=1;i<row+1;i++)
	{
		...
		mid_push_unregist(result[i*column]);
	}
	
	return 0;
}
static int unregist_allbrand(void)
{
	char sqlite_cmd[256+128];
	int (*sqlite_callback)(char **, int, int, void *) = brand_unregist_sqlite_callback;

	snprintf(sqlite_cmd,sizeof(sqlite_cmd),"SELECT regist_dir FROM brand;");
...
static int pid_unregist_sqlite_callback(char **result, int row, int column, void *receiver)
{
	...
	for(i=1;i<row+1;i++)
	{
		...
		free_filter((unsigned short)(pid));
