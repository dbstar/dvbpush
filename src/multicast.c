...
void *igmp_join_thread(void *multi_addr)
{
	...
	while(1){
		...
		multicast_failed_sleep = multicast_failed_sleep<<2;
		if(multicast_failed_sleep>60*60){
			DEBUG("join multicast failed, go out\n");
			ret = -1;
			goto END;
		}
		else{
			DEBUG("sleep %d for next multicast join\n", multicast_failed_sleep);
			sleep(multicast_failed_sleep);
		}
	}
...
//		printf("left: %d\n", left);
		...
		parse_ts_packet(p_buf,p_write,&p_read);
...
int softdvb_init()
{
	...
	// prog/video
	unsigned short video_pid = 102;	//prog_data_pid_get();
	int filter2 = alloc_filter(102);
